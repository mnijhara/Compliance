...
import { AlertTriangle, ArrowRight, CheckCircle2, FileText, RefreshCw, Sparkles, Upload, X } from 'lucide-react';
...
export const QuickStart: React.FC<{ onChat: () => void }> = () => {
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8" onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); void readFiles(Array.from(e.dataTransfer.files)); }}>
        <div className="grid gap-8 lg:grid-cols-[1fr_280px] lg:items-center">
          <div>
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-600">Document review</div>
            <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Upload the HR documents you already have.</h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">Drop policies, contracts and HR documents here. Start with the documents; add jurisdiction and establishment context later when it matters.</p>
            <div className={`mt-6 rounded-2xl border-2 border-dashed p-7 text-center transition ${dragging ? 'border-indigo-500 bg-indigo-50' : 'border-slate-300 bg-slate-50'}`}>
              ...
              <h3 className="mt-3 text-base font-extrabold text-slate-900">Drop files here</h3>
              <p className="mt-1 text-xs text-slate-500">PDF, TXT, MD, CSV, JSON or HTML · up to 5 files</p>
              <button onClick={() => inputRef.current?.click()} className="mt-5 rounded-xl bg-slate-950 px-5 py-3 text-xs font-bold text-white">Choose files</button>
            </div>
          </div>
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-5">
            <div className="flex items-center gap-2 text-sm font-extrabold text-slate-900"><Sparkles className="h-4 w-4 text-indigo-600" /> What happens next?</div>
            <div className="mt-4 space-y-3 text-xs leading-5 text-slate-600"><div><strong className="text-slate-900">01 · Read</strong><br />Your uploaded content is sent for review.</div><div><strong className="text-slate-900">02 · Review</strong><br />Potential issues are separated from missing evidence.</div><div><strong className="text-slate-900">03 · Act</strong><br />You get suggested next actions to verify.</div></div>
            <p className="mt-4 text-[10px] leading-4 text-slate-500">Results are not legal advice and material conclusions require source verification.</p>
          </div>
        </div>

        {files.length > 0 && <div className="mt-6 space-y-3"><div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">Ready to review</span><button onClick={() => setFiles([])} className="text-xs font-semibold text-slate-500 hover:text-slate-900">Clear</button></div>{files.map(file => <div key={file.name} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><FileText className="h-4 w-4 text-indigo-600" /><span className="flex-1 truncate text-sm font-semibold text-slate-800">{file.name}</span><span className="text-[10px] text-slate-400">{Math.ceil(file.size / 1024)} KB</span><button onClick={() => setFiles(prev => prev.filter(item => item.name !== file.name))} className="text-slate-400 hover:text-slate-800"><X className="h-4 w-4" /></button></div>)}<button disabled={!canRun} onClick={() => void runReview()} className="w-full rounded-xl bg-slate-950 px-5 py-3.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{running ? <span className="inline-flex items-center gap-2"><RefreshCw className="h-4 w-4 animate-spin" /> Nova is reviewing your documents…</span> : <span className="inline-flex items-center gap-2"><Sparkles className="h-4 w-4" /> Review my documents <ArrowRight className="h-4 w-4" /></span>}</button></div>}
